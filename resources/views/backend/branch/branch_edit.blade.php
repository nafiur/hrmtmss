    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <a href="{{ route('branch.all') }}" class="btn btn-dark btn-rounded waves-effect waves-light"
                        ...
                        <form method="post" action="{{ route('branch.update') }}" id="myForm">
                                ...
                                <div class="col-sm-10">
                                    <select name="domain_id" class="form-select" aria-label="Default select example">
                                        ...
                                        @foreach ($domain as $doma)
                                            <option value="{{ $doma->id }}"
                                                {{ $doma->id == $branch->domain_id ? 'selected' : '' }}>
                                                {{ $doma->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select name="zone_id" class="form-select" aria-label="Default select example">
                                        ...
                                        @foreach ($zone as $zone)
                                            <option value="{{ $zone->id }}"
                                                {{ $zone->id == $branch->zone_id ? 'selected' : '' }}>
                                                {{ $zone->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select name="area_id" class="form-select" aria-label="Default select example">
                                        ...
                                        @foreach ($area as $area)
                                            <option value="{{ $area->id }}"
                                                {{ $area->id == $branch->area_id ? 'selected' : '' }}>
                                                {{ $area->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="form-group col-sm-10">
                                    <input name="name" class="form-control" value="{{ $branch->name }}"
                                        type="text">
                                </div>
                            </div>
                            <!-- end row -->
