    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <a href="{{ route('zone.all') }}" class="btn btn-dark btn-rounded waves-effect waves-light"
                        ...
                        <form method="post" action="{{ route('zone.update') }}" id="myForm">
                                    ...
                                    <select name="domain_id" class="form-select" aria-label="Default select example">
                                        ...
                                        @foreach ($domain as $domain)
                                            <option value="{{ $domain->id }}"
                                                {{ $domain->id == $zone->domain_id ? 'selected' : '' }}>
                                                {{ $domain->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="form-group col-sm-10">
                                    <input name="name" class="form-control" value="{{ $zone->name }}"
                                        type="text">
                                </div>
                            </div>
                            <!-- end row -->
