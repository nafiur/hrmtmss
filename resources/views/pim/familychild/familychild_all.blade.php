    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="px-4 py-3 rounded breadcrumb breadcrumb-alt bg-body-extra-light push fs-sm">
                ...
                <li class="breadcrumb-item active" aria-current="page"><a
                        href="{{ route('all.familychild') }}">Family/Child</a>
                </li>
            </ol>
        </nav>
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                <a href="{{ route('add.familychild') }}" class="btn btn-dark btn-rounded waves-effect waves-light"
                    style="float:right;"><i class="fa fa-plus-circle"> Add New Child </i></a> <br> <br>
            </div>
            <div class="mb-5 block-content">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap"
                    ...
                    <tbody>
                        @foreach ($familychild as $key => $item)
                            <tr>
                                <td> {{ $key + 1 }} </td>
                                ...
                                <td>
                                    {{-- @if (Auth::user()->can('familychild.show')) --}}
                                        <a href="{{ route('show.familychild', $item->id) }}" class="btn btn-info sm"
                                            title="Show Data"> <i class="fas fa-eye"></i> </a>
                                    {{-- @endif --}}

                                    {{-- @if (Auth::user()->can('ewemployee.export')) --}}
                                        <a href="{{ route('delete.familychild', $item->id) }}"
                                            class="btn btn-success sm" title="Delete Data" id="delete"> <i
                                                class="fas fa-download"></i> </a>
                                    {{-- @endif --}}

                                    {{-- @if (Auth::user()->can('familychild.edit')) --}}
                                        <a href="{{ route('edit.familychild', $item->id) }}" class="btn btn-info sm"
                                            title="Edit Data"> <i class="fas fa-edit"></i> </a>
                                    {{-- @endif --}}

                                    {{-- @if (Auth::user()->can('familychild.delete')) --}}
                                        <a href="{{ route('delete.familychild', $item->id) }}"
