    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <a href="{{ route('area.all') }}" class="btn btn-dark btn-rounded waves-effect waves-light"
                        ...
                        <form method="post" action="{{ route('area.update') }}" id="myForm">
                                    ...
                                    <select name="domain_id" class="form-select" aria-label="Default select example">
                                        ...
                                        @foreach ($domain as $doma)
                                            <option value="{{ $doma->id }}"
                                                {{ $doma->id == $area->domain_id ? 'selected' : '' }}>
                                                {{ $doma->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select name="zone_id" class="form-select" aria-label="Default select example">
                                        ...
                                        @foreach ($zone as $zone)
                                            <option value="{{ $zone->id }}"
                                                {{ $zone->id == $area->zone_id ? 'selected' : '' }}>{{ $zone->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="form-group col-sm-10">
                                    <input name="name" class="form-control" value="{{ $area->name }}"
                                        type="text">
                                </div>
                            </div> <!-- end row -->
